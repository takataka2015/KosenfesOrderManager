...
function appendHistory(obj: any) {
    fs.appendFileSync(FilePath.History, JSON.stringify(obj, null, 1) + ",\n", "utf-8");
}

export function runOnce() {
    // コンソール確認用
    console.log("[autoSweep] runOnce triggered at", new Date().toLocaleTimeString());
    const reqObj = JSON.parse(fs.readFileSync(FilePath.Request, "utf-8")) as { serials: number[] };
    ...
    saveNow(nowArr);
    fs.writeFileSync(FilePath.Request, JSON.stringify({ serials: remained }, null, 1), "utf-8");
}
...
export function startAutoSweep(intervalMs = 1000) {
    ...
    g[GLOBAL_KEY] = setInterval(() => {
        try { runOnce(); } catch (e) { console.error("auto-sweep error:", e); }
    }, intervalMs);
}
